...
@MainActor final class TmuxController {
    ...
    ///
    /// Unlike `kill()`, the teardown is IMMEDIATE and does not wait for tmux to answer: the enqueued
    /// `detach-client` write races the synchronous `PTYProcess.terminate()` and usually loses — but for a
    /// detach that is harmless, because killing the transport IS a detach (the ssh/tmux client dies, the
    /// server sees the client go and detaches it), and the outcome is identical either way. `kill()` has
    /// no such equivalence — nothing but the delivered `kill-session` ends the remote session — which is
    /// why it alone defers teardown to the `%exit` echo.
    func detach() {
