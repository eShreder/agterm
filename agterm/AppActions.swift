...
final class AppActions {
    ...
    func closeActiveSession() -> Bool {
        ...
        closeSession(session.id)   // backend-aware: a tmux session routes to kill-window
        focusActiveSession()
        return true
    }
    ...
    /// Close a session — backend-aware (frontmost store). A tmux-backed session routes to `kill-window`
    /// via `closeTmuxSession`; a normal session closes locally. Used by `closeActiveSession` (the ⌘W path,
    /// which confirms first); the sidebar context menu uses `closeSession(_:in:)` (confirms + resolves its
    /// own window-local store) and the `session.close` control arm resolves its store explicitly.
