...
extension AppActions {
    ...
    /// `newSession()`'s tmux leg: in a tmux-backed session, "New Session" creates a new WINDOW in the
    /// attached tmux session (like iTerm2 ⌘T) — it comes back as a `%window-add` and becomes an agterm
    /// session. Returns whether the active session was tmux-backed (and so the request was routed to
    /// tmux); false leaves the caller on its normal path, a local shell in the current workspace.
    func newTmuxWindowForActiveSession() -> Bool {
        guard let store, let active = store.activeSession, active.tmuxBinding != nil,
              let controller = tmuxControllerOwning(active.id) else { return false }
        controller.newWindow()
        return true
    }

    /// `deleteWorkspace`'s tmux leg. A tmux mirror is an ephemeral workspace backed by a live
    /// `TmuxController`; route its delete to a DETACH (which removes the workspace via teardown) so the
    /// gateway + relay sockets are freed, not orphaned — its shells survive server-side, hence the
    /// detach-worded confirm (the standard "ends their running shells" one would misinform). Returns
    /// whether `workspace` was a mirror and so fully handled here — including the cancelled-confirm case,
    /// which must not fall through to the normal delete. A normal workspace has no controller and returns
    /// false; a mirror whose controller is already torn down is removed plainly.
    func deleteTmuxMirror(_ workspace: Workspace, in store: AppStore) -> Bool {
        guard workspace.ephemeral else { return false }
        if !workspace.sessions.isEmpty, !confirmDetachTmux(workspace) { return true }
        if !detachTmux(forConnectionWorkspace: workspace.id) {
            store.removeWorkspace(workspace.id) // mirror with no live controller (already torn down)
        }
        return true
    }
    ...
    /// wording must not claim the shells end. Returns whether the user confirmed. Internal rather than
    /// private because its callers live in `AppActions.swift` / this file's `deleteTmuxMirror`.
    func confirmDetachTmux(_ workspace: Workspace) -> Bool {
        let alert = NSAlert()
