...
struct TmuxGatewayTests {
    @Test func splitsBootstrapFromControlAndParsesEvents() async throws {
        ...
        // A generous deadline on purpose: the wait returns the instant the condition holds, so the only
        // thing a big number buys is tolerance for a loaded machine — swift-testing runs suites in
        // parallel, and spawning the transcript subprocess there can take seconds.
        try await waitUntil(20.0) { events.contains(.exit(reason: nil)) }
        ...
        try await waitUntil(20.0) { exited.isSet }
        #expect(exited.isSet)
        gw.stop()
    }
