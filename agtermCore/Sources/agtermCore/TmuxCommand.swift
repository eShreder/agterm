...
public enum TmuxCommandEncoder {
    ...
    ///
    /// The filter runs over UNICODE SCALARS, not `Character`s, and must stay that way: Swift groups a
    /// CRLF pair into ONE grapheme cluster, so a `Character`-level `$0 != "\n" && $0 != "\r"` compares
    /// that cluster against each half, matches neither, and passes `\r\n` through intact — the one line
    /// break an attacker would actually type. Scalars see the CR and the LF separately.
    static func quote(_ arg: String) -> String {
        let oneLine = String(String.UnicodeScalarView(arg.unicodeScalars.filter { $0 != "\n" && $0 != "\r" }))
