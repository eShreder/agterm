...
public final class PTYProcess: @unchecked Sendable {
    ...
    /// The window size stamped on the pty before the spawn. A fresh `posix_openpt` starts at 0x0, and a
    /// control-mode child inherits that as its CLIENT size — so `tmux -CC new` would create a session
    /// sized from a degenerate tty (and an attach would briefly drive every window from it) until the
    /// first mirrored surface's `refresh-client -C` lands. The conventional 80x24 fallback keeps that
    /// window sane; the real geometry follows within a frame, so this is a floor, not a guess at the
    /// user's terminal. NOT a `start()` parameter: no caller knows better at spawn time (the mirrored
    /// surfaces do not exist yet), and `start` already sits on swiftlint's parameter-count threshold.
    private static let initialWindowSize = (cols: UInt16(80), rows: UInt16(24))
    ...
    public func start(path: String, args: [String], env: [String: String],
        ...
        // Seed the window size (see `initialWindowSize`). Set on the PRIMARY, the end a terminal
        // emulator owns; the pty carries one size for both ends, so the child reads it back through
        // TIOCGWINSZ on its own tty. Done BEFORE the spawn, so there is no process group to signal yet
        // and no SIGWINCH is owed — the child simply starts with a sane size instead of 0x0.
        var ws = winsize(ws_row: Self.initialWindowSize.rows, ws_col: Self.initialWindowSize.cols,
                         ws_xpixel: 0, ws_ypixel: 0)
        _ = ioctl(primary, UInt(TIOCSWINSZ), &ws)
        ...
        ps.setEventHandler { [self] in
            ...
            pid = -1
            pidLock.unlock()
            // Drain whatever the child wrote before exiting, BEFORE `onExit` and the cancel below. This
            // source and the read source share one serial queue, so when a short-lived child's exit is
            // delivered first, cancelling the read source here would close the fd with the child's final
            // bytes still sitting in the pty — dropping, say, ssh's last "Permission denied" line or a
            // tmux `%exit`. Switch to non-blocking first so the drain returns EAGAIN instead of wedging
            // the queue should any other process still hold the pts open.
            let flags = self.primaryFD >= 0 ? fcntl(self.primaryFD, F_GETFL, 0) : -1
            // Only drain once the fd is CONFIRMED non-blocking: a blocking read here would wedge this
            // serial queue (and with it every later write/teardown) if any process still held the pts
            // open. Losing a tail on the fcntl-failed path is the cheap side of that trade.
            if flags >= 0, fcntl(self.primaryFD, F_SETFL, flags | O_NONBLOCK) == 0 {
                var drain = [UInt8](repeating: 0, count: 65536)
                while true {
                    let n = read(self.primaryFD, &drain, drain.count)
                    if n > 0 { onData(Data(drain[0..<n])) } else { break }
                }
            }
            onExit(status)
