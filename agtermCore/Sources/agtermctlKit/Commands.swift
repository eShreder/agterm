...
struct BasicOptions: ParsableArguments, ConnectionOptions {
    ...
    /// Resolve the socket path, in precedence order: `--socket` → `AGTERM_CONTROL_SOCKET` →
    /// `<AGTERM_STATE_DIR>/agterm.sock` → `<$HOME>/Library/Application Support/agterm/agterm.sock` →
    /// `/tmp/agterm/agterm.sock`. `env` is injectable so the precedence is unit-testable; production
...
public struct Agtermctl: ParsableCommand {
    public static let configuration = CommandConfiguration(
        ...
        subcommands: [Tree.self, Events.self, Workspace.self, Session.self, Surface.self, Dashboard.self, Window.self, Quick.self,
                      ...
                      Zmx.self, Version.self, Tmux.self, TmuxPipe.self]
    )
