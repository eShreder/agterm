...
- `control-api.md` — the full 64-command control catalog (incl. `tmux.*`), the three protocol layers,
  addressing, the CLI/hooks/skill installers, and the tmux relay/engine
  (`-CC` demux, per-window relay bridge, backend-aware `session.*`, ephemeral mirror workspace, PID-reuse).
  ...
  the tmux engine (`agterm/Tmux/`, host-free `Tmux*`/`RelayProtocol`/`PTYProcess`),
